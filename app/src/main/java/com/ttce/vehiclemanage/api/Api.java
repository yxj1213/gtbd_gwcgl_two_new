...
public class Api {
        ...
        Interceptor headerInterceptor = new Interceptor() {
            ...
            public Response intercept(Chain chain) throws IOException {
                ...
                if (UserManager.isLogin()) {
                        build = builder1.addHeader("Authorization", "Bearer " + UserManager.getLoginBean().getToken())
                                .build();
                } else {
                    build = builder1.addHeader("Authorization", "").build();
                }
                return chain.proceed(build);
            }
        };

        okHttpClient = new OkHttpClient.Builder().readTimeout(READ_TIME_OUT, TimeUnit.MILLISECONDS)
                .connectTimeout(CONNECT_TIME_OUT, TimeUnit.MILLISECONDS).addInterceptor(mRewriteCacheControlInterceptor)
                .addNetworkInterceptor(mRewriteCacheControlInterceptor).addInterceptor(headerInterceptor)
                .addInterceptor(logInterceptor).cache(cache).build();

        Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd'T'HH:mm:ssZ").serializeNulls().create();
        retrofit = new Retrofit.Builder().client(okHttpClient).addConverterFactory(GsonConverterFactory.create(gson))
                .addCallAdapterFactory(RxJavaCallAdapterFactory.create()).baseUrl(ApiConstants.getHost(hostType))
                .build();
    ...
    public static ApiService getDefault(int hostType) {
        ...
        if(ApiConstants.HOST_CHANGE) {
            retrofitManager = new Api(hostType);
            sRetrofitManager.put(hostType, retrofitManager);
            ApiConstants.HOST_CHANGE = false;
        }
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            if (!NetWorkUtils.isNetConnected(BaseApplication.getAppContext())) {
                request = request.newBuilder()
                        .cacheControl(
                                TextUtils.isEmpty(cacheControl) ? CacheControl.FORCE_NETWORK : CacheControl.FORCE_CACHE)
                        .build();
            }
