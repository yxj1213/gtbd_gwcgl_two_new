...
import com.ttce.vehiclemanage.utils.ColorUtils;
...
public class NewSwitchButton extends CompoundButton {
    ...
    public NewSwitchButton(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = attrs == null ? null : getContext().obtainStyledAttributes(attrs, R.styleable.NewSwitchButton);
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.NewSwitchButton_kswThumbDrawable);
            thumbColor = ta.getColorStateList(R.styleable.NewSwitchButton_kswThumbColor);
            margin = ta.getDimension(R.styleable.NewSwitchButton_kswThumbMargin, margin);
            marginLeft = ta.getDimension(R.styleable.NewSwitchButton_kswThumbMarginLeft, margin);
            marginRight = ta.getDimension(R.styleable.NewSwitchButton_kswThumbMarginRight, margin);
            marginTop = ta.getDimension(R.styleable.NewSwitchButton_kswThumbMarginTop, margin);
            marginBottom = ta.getDimension(R.styleable.NewSwitchButton_kswThumbMarginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.NewSwitchButton_kswThumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.NewSwitchButton_kswThumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.NewSwitchButton_kswThumbRadius, Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.NewSwitchButton_kswBackRadius, thumbRadius + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.NewSwitchButton_kswBackDrawable);
            backColor = ta.getColorStateList(R.styleable.NewSwitchButton_kswBackColor);
            backMeasureRatio = ta.getFloat(R.styleable.NewSwitchButton_kswBackMeasureRatio, backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.NewSwitchButton_kswAnimationDuration, animationDuration);
            fadeBack = ta.getBoolean(R.styleable.NewSwitchButton_kswFadeBack, true);
            tintColor = ta.getColor(R.styleable.NewSwitchButton_kswTintColor, tintColor);
            textOn = ta.getString(R.styleable.NewSwitchButton_kswTextOn);
            textOff = ta.getString(R.styleable.NewSwitchButton_kswTextOff);
            textMarginH = Math.max(textMarginH, backRadius / 2);
            textMarginH = ta.getDimension(R.styleable.NewSwitchButton_kswTextMarginH, textMarginH);
            autoAdjustTextPosition = ta.getBoolean(R.styleable.NewSwitchButton_kswAutoAdjustTextPosition, autoAdjustTextPosition);
            ta.recycle();
        }
