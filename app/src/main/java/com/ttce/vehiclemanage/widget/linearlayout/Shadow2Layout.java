 ...
 *
 */
public class Shadow2Layout extends LinearLayout {


    private static  final  String TAG = "ShadowLayout";

    //默认阴影半径
    public static final float SHADOW_DEFAULT_RADIUS = DeviceUtils.dip2px(BaseApplication.getAppContext(),5);

    //阴影最大偏移量
    public static final float SHADOW_MAX_OFFSET = DeviceUtils.dip2px(BaseApplication.getAppContext(),20);

    //阴影最大模糊半径
    public static final float SHADOW_MAX_BLUR = DeviceUtils.dip2px(BaseApplication.getAppContext(),20);



    //默认模糊半径
    public static final float SHADOW_DEFAULT_BLUR_RADIUS = DeviceUtils.dip2px(BaseApplication.getAppContext(),5);
    ...
    //水平位移
    private float xOffset  =DeviceUtils.dip2px(BaseApplication.getAppContext(),10);


    //竖直方向位移
    private float yOffset = DeviceUtils.dip2px(BaseApplication.getAppContext(),10);
    ...
    public Shadow2Layout(Context context) {
        super(context,null);
    }

    public Shadow2Layout(Context context, AttributeSet attrs) {
        this(context, attrs,0);
    }

    public Shadow2Layout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        xOffset = typedArray.getDimension(R.styleable.ShadowLayout_xOffset,DeviceUtils.dip2px(BaseApplication.getAppContext(),10));
        yOffset = typedArray.getDimension(R.styleable.ShadowLayout_yOffset,DeviceUtils.dip2px(BaseApplication.getAppContext(),10));
...
//        mPaint.setShadowLayer(blurRadius,0,0,shadowColor);
    ...
    class ShadowConfig implements Shadow{
        ...
        //代理
        private Shadow2Layout shadow;

        private ShadowConfig(Shadow2Layout shadow) {
            this.shadow = shadow;
        }
