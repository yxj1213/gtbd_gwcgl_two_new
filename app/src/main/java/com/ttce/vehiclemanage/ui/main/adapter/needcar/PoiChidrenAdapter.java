...
public class PoiChidrenAdapter  extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        String name=mPoiChildrenInfoList.get(position).getShowName();
        if(name.length()<=4){
            name=mPoiChildrenInfoList.get(position).getShowName();;
        }else{
            name=name.substring(0,4)+"...";
        }
        viewHolder.poichildrenShowName.setText(name);
        viewHolder.poichildrenShowName.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mOnGetChildrenLocationListener.getChildrenLocation(poiChildrenInfo);
            }
        });
