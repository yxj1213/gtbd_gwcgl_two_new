    ...
    public final List<OverlayOptions> getOverlayOptions() {
            ...
            for (DrivingStep step : mRouteLine.getAllStep()) {
                ...
                if (step.getEntrance() != null) {
                    overlayOptionses.add((new MarkerOptions())
                            ...
                            .extraInfo(b)
                            .icon(BitmapDescriptorFactory.fromResource(R.mipmap.icon_line_node)).visible(false));
                }
                // 最后路段绘制出口点
        ...
        if (mRouteLine.getStarting() != null) {
            overlayOptionses.add((new MarkerOptions())
                    ...
                    .icon(BitmapDescriptorFactory.fromResource(R.mipmap.icon_begin)).zIndex(10));
        }
        ...
        if (mRouteLine.getWayPoints() != null&&mRouteLine.getWayPoints().size()>0) {
            for (RouteNode r: mRouteLine.getWayPoints()) {
                if(!mlist.contains(r.getLocation())){
                    mlist.add(r.getLocation());
                    overlayOptionses.add((new MarkerOptions())
...
//            }
            ...
            PolylineOptions option = new PolylineOptions().points(points).textureIndex(traffics)
                    .width(DisplayUtil.dip2px(3)).dottedLine(true).focus(false)
                    .color(BaseApplication.getAppContext().getResources().getColor(R.color.color_f0)).zIndex(0);
            if (isDotLine) {
