...
package com.ttce.vehiclemanage.utils.clustering;
...
import com.ttce.vehiclemanage.utils.clustering.algo.Algorithm;
import com.ttce.vehiclemanage.utils.clustering.algo.NonHierarchicalDistanceBasedAlgorithm;
import com.ttce.vehiclemanage.utils.clustering.algo.PreCachingAlgorithmDecorator;
import com.ttce.vehiclemanage.utils.clustering.view.ClusterRenderer;
import com.ttce.vehiclemanage.utils.clustering.view.DefaultClusterRenderer;
 ...
 * <li>
 */
public class ClusterManager<T extends ClusterItem & Serializable> implements
    ...
    public ClusterManager(Context context, BaiduMap map, MarkerManager markerManager) {
        ...
        mAlgorithm =
                new PreCachingAlgorithmDecorator<T>(new NonHierarchicalDistanceBasedAlgorithm<T>(mMap));
    ...
    /**
     * Force a re-cluster. You may want to call this after adding new item(s).强制重新聚集。添加新项目后，您可能需要调用此选项。
     */
    public void cluster() {
...
//TODO 1、优化聚合点功能（注释掉如下代码）这段代码 的意思是地图zoom不发生变化时，将不调用后面的方法。
...
//    }
    ...
    /**
     * Runs the clustering algorithm in a background thread, then re-paints when results come back.在后台线程中运行聚类算法，然后在结果返回时重新绘制。
     */
...
//        mRenderer.setOnClusterItemClickListener(listener);
    ...
    /**
     * Called when a Cluster is clicked.单击群集时调用。
     */
    ...
    /**
     * Called when a Cluster's Info Window is clicked.单击群集窗口时调用。
     */
    ...
    /**
     * Called when an individual ClusterItem is clicked.单击单个ClusterItem时调用。
     */
    ...
    /**
     * Called when an individual ClusterItem's Info Window is clicked.单击单个ClusterItem的信息窗口时调用。
     */
